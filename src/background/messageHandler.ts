...
export const messageHandler = async (message: IncomingMessage): Promise<any> => {
    ...
    switch (type) {
        ...
        // CONTENT_MESSAGES has no PING member, so this case evaluates to
        // `undefined` and is unreachable — a pre-existing quirk preserved
        // via the cast, not fixed.
        case (CONTENT_MESSAGES as Record<string, string | undefined>).PING: {
            break;
        }
