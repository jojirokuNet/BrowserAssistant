...
const CurrentSite = observer(() => {
    ...
    const getHandlerForHttpsSite = (handler: CertModalEventHandler): CertModalEventHandler | undefined => {
    ...
    const getHandlerForHttpSite = (handler: CertModalEventHandler): CertModalEventHandler | undefined => {
        if (!pageProtocol.isHttps) {
            return handler;
        }
        return undefined;
    ...
    const handleCertStatusModalState: CertModalEventHandler = (event, payload) => {
        updateCertStatusModalState(event.type, payload);
    };

    const onKeyEnterDown = (event: React.KeyboardEvent<HTMLDivElement>) => {
    ...
    const handleSecureStatusModalState: CertModalEventHandler = (event, payload) => {
    ...
    const onKeyEnterDownSecure = (event: React.KeyboardEvent<HTMLDivElement>) => {
        if (event.key !== 'Enter') {
            return;
        }
