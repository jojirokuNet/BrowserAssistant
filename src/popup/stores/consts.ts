...
/**
 * Modal interaction flags keyed by modal state name; the record type
 * keeps the reduce below writable under strict mode.
 */
export type ModalState = Record<string, boolean>;

export const DEFAULT_MODAL_STATE: ModalState = Object.values(MODAL_STATES_NAMES)
    .reduce((acc: ModalState, name) => {
        acc[name] = false;
        return acc;
    }, {} as ModalState);

export const EVENT_TYPE_TO_MODAL_STATE_MAP: Record<string, ModalState> = {
